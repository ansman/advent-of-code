...
fun energize(startingBeam: Beam): Int {
    val beams = LinkedList(listOf(startingBeam))
...
emptySequence<Beam>()
        .plus(grid[0].indices.asSequence().flatMap { sequenceOf(Beam(it, 0, Beam.Direction.Down), Beam(it, grid.lastIndex, Up)) })
        .plus(grid.indices.asSequence().flatMap { sequenceOf(Beam(0, it, Beam.Direction.Right), Beam(grid[it].lastIndex, it, Beam.Direction.Left)) })
        .map { energize(it) }
        .max()
        .let(::println)
